...
quizbot was written in Python and runs on the 2.7 interpreter. The dependencies
...
The 3.x interpreter will also be able to run this. In order to *enforce* using a 2.x
interpreter (2.7.x is the one with which quizbot is formally tested) this
may be accomplished by specifically invoking a 2.x interpreter on some systems.
`$ python2 q`
